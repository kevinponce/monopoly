...
describe MovePlayerService do
  ...
  describe 'visiting jail' do
    ...
    before(:each) { MovePlayerService.call(game: game, player: player, dice: [1, 2]) }
  ...
  describe 'chance' do
    ...
    before(:each) { MovePlayerService.call(game: game, player: player, dice: [4, 3]) }
  ...
  describe 'passed go community_chest get of jail' do
    ...
    before(:each) { MovePlayerService.call(game: game, player: player, dice: [6, 6]) }
  ...
  describe 'property not owned' do
    ...
    before(:each) { MovePlayerService.call(game: game, player: player, dice: [6, 1]) }
  ...
  describe 'got to jail' do
    let!(:player) { create(:player, game: game, position: 25) }

    let!(:go_to_jail) { create(:place, :go_to_jail) }
    let!(:jail) { create(:place, :jail) }

    before(:each) { MovePlayerService.call(game: game, player: player, dice: [2, 3]) }

    it { expect(player.reload.money).to eq(3000) }
    it { expect(player.reload.position).to eq(jail.order) }
    it { expect(player.reload.in_jail).to be_truthy }
  end
  ...
  describe 'free parking' do
    ...
    before(:each) { MovePlayerService.call(game: game, player: player, dice: [1, 1]) }
  ...
  describe 'income_tax' do
    ...
    before(:each) { MovePlayerService.call(game: game, player: player, dice: [2, 2]) }
  ...
  describe 'luxury_tax' do
    ...
    before(:each) { MovePlayerService.call(game: game, player: player, dice: [3, 3]) }
